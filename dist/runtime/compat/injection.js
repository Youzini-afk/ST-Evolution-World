 ...
 */
export function injectReplyInstruction(content) {
    if (!content.trim())
        return;
    ...
    ctx.setExtensionPrompt(EW_INJECT_KEY, content.trim(), EXTENSION_PROMPT_IN_CHAT, 0, // depth
 ...
 * 清除已注入的回复指令。
 */
export function clearReplyInstruction() {
    try {
        const ctx = getSTContext();
        if (typeof ctx.setExtensionPrompt === "function") {
            ctx.setExtensionPrompt(EW_INJECT_KEY, "", EXTENSION_PROMPT_IN_CHAT, 0);
