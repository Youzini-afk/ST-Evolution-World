...
// 辅助渲染函数
// ────────────────────────────────────────────────────
function renderGetwiLines(entries, indent = '') {
    if (entries.length === 0) {
...
function renderDecorators(decorators) {
    if (decorators.length === 0)
        return '';
    return decorators.join('\n') + '\n';
}
/** getvar 声明 */
function renderVariableDeclarations(model) {
...
/** setvar 调用 */
function renderSetVariables(model) {
    if (model.set_variables.length === 0)
        return '';
...
/** 纯文本注入 */
function renderInjectText(model) {
    if (model.inject_text.length === 0)
        return '';
    return model.inject_text.join('\n') + '\n';
}
/** activewi 调用 */
function renderActivateEntries(model) {
    if (model.activate_entries.length === 0)
        return '';
...
/** 角色检测 + 动态加载 */
function renderCharDetection(model, dynPrefix) {
...
/** for_each — 通用列表循环加载 */
function renderForEach(model, dynPrefix) {
    if (model.for_each.length === 0)
        return '';
    let code = '';
...
function renderRulesAndFallback(model) {
    let body = '';
...
// 校验
// ────────────────────────────────────────────────────
function fallbackSyntaxCheck(content) {
...
export async function validateEjsTemplate(content) {
...
// 主渲染入口
// ────────────────────────────────────────────────────
export async function renderControllerTemplate(model, dynPrefix = 'EW/Dyn/') {
...
//# sourceMappingURL=controller-renderer.js.map