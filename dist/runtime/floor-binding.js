import { getChatId, getChatMessages, getCurrentCharacterName, getLastMessageId, setChatMessages, } from "./compat/character";
import { EVENT_CHAT_CHANGED, EVENT_MESSAGE_DELETED, onEvent, } from "./compat/events";
...
import { buildChatFingerprint, buildFileName, buildFilePrefix, buildLegacyFileName, buildLegacyFilePrefix, buildSnapshotStoreOwner, cleanupSnapshotFiles, deleteSnapshot, FILE_ARTIFACT_VERSION, hasSnapshotStorePayload, readSnapshotStore, writeSnapshot, writeSnapshotStore, } from "./snapshot-storage";
import { buildDynSnapshotFromEntry, ensureDefaultEntry, normalizeDynSnapshotData, resolveTargetWorldbook, } from "./worldbook-runtime";
...
function normalizeDynSnapshot(snapshot) {
...
function controllerSnapshotKey(snapshot) {
    return String(snapshot.flow_id ?? snapshot.entry_name ?? snapshot.flow_name ?? "legacy");
}
const floorBindingListenerStops = [];
const observedMessageVersionKeys = new Map();
let floorBindingRestoreTimer = null;
const localizationInFlightByChatKey = new Map();
const localizationSignatureByChatKey = new Map();
function scheduleFloorBindingRestore(getSettings, delayMs) {
...
function clearInlineSnapshotFields(data) {
...
function clearFloorSnapshotFields(data) {
...
function refreshObservedMessageVersions() {
...
function hasSnapshotMetadataHints(msg) {
    const data = (msg?.data ?? {});
    return Boolean(data[EW_SNAPSHOT_FILE_KEY] ||
...
function getMessageSnapshotFileCandidates(msg) {
    const candidates = [];
    ...
    if (hasSnapshotMetadataHints(msg) &&
...
function isSnapshotFileNamedForCurrentChat(fileName) {
...
function isSnapshotOwnerMatchingCurrentChat(owner) {
    if (!owner) {
        return false;
    }
    const expected = buildSnapshotStoreOwner(getCharName(), getChatId());
    return (owner.char_name === expected.char_name &&
        owner.chat_id === expected.chat_id &&
        owner.chat_fingerprint === expected.chat_fingerprint);
}
function isSnapshotStoreOwnedByCurrentChat(fileName, store) {
...
function buildSnapshotReadResult(source, resolution, snapshot, matchedVersionKey) {
...
function getVersionEntries(versions) {
    return Object.entries(versions);
}
function selectSnapshotFromSources(sources, versionInfo, mode) {
...
function readLegacyInlineSnapshot(data) {
    const snapshots = _.get(data, EW_DYN_SNAPSHOTS_KEY);
    const controllersArray = _.get(data, EW_CONTROLLERS_DATA_KEY);
    const inlineSwipeId = typeof data[EW_SWIPE_ID_KEY] === "number"
        ? data[EW_SWIPE_ID_KEY]
        : undefined;
    const inlineContentHash = typeof data[EW_CONTENT_HASH_KEY] === "string"
        ? data[EW_CONTENT_HASH_KEY]
        : undefined;
    ...
    const controllersRaw = _.get(data, EW_CONTROLLERS_DATA_KEY);
    if (controllersRaw &&
        typeof controllersRaw === "object" &&
        !Array.isArray(controllersRaw)) {
        return {
            controllers: Object.entries(controllersRaw).map(([key, value]) => normalizeControllerSnapshot({
    ...
    const ctrlSnap = _.get(data, EW_CONTROLLER_DATA_KEY);
...
function normalizeInlineSnapshotVersions(raw) {
    ...
    for (const [key, value] of Object.entries(raw)) {
        ...
        versions[String(key)] = {
            ...
            dyn_entries: Array.isArray(upgraded.dyn_entries)
                ? upgraded.dyn_entries.map((entry) => normalizeDynSnapshot(entry))
                : [],
            swipe_id: typeof upgraded.swipe_id === "number" ? upgraded.swipe_id : undefined,
            content_hash: typeof upgraded.content_hash === "string"
...
function readInlineSnapshotVersions(data) {
...
function readInlineSnapshot(data, versionKey) {
...
function writeInlineSnapshotVersions(nextData, versions) {
    nextData[EW_INLINE_SNAPSHOT_VERSIONS_KEY] = versions;
}
function buildSnapshotStoreFromVersions(versions) {
    return {
...
async function loadSnapshotVersionSources(msg) {
    const sources = [];
...
async function readSnapshotForMessageDetailed(msg, mode) {
...
async function readSnapshotForMessage(msg) {
...
function isEmptySnapshotPayload(snapshot) {
    ...
    return ((snapshot.controllers?.length ?? 0) === 0 &&
        (snapshot.dyn_entries?.length ?? 0) === 0);
}
export async function repairCurrentChatSuspiciousEmptySnapshots() {
    const warnings = [];
    let repaired = 0;
    const lastId = getLastMessageId();
    if (lastId < 0) {
    ...
    for (const msg of allMessages) {
        ...
        if (!executionState || !executionState.workflow_failed) {
            continue;
        }
        ...
        if (!fallbackReadResult.snapshot ||
            isEmptySnapshotPayload(fallbackReadResult.snapshot)) {
            continue;
        }
        const nextData = {
            ...(msg.data ?? {}),
        ...
        for (const source of exactEmptySources) {
            ...
            try {
                ...
                if (!store ||
                    !isEmptySnapshotPayload(store.versions[currentVersionKey])) {
                    continue;
                }
                ...
                warnings.push(`message #${msg.message_id} file ${source.fileName}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        if (!mutated) {
            continue;
        ...
        const explicitFileName = typeof nextData[EW_SNAPSHOT_FILE_KEY] === "string"
...
export async function pinMessageSnapshotToCurrentVersion(messageId) {
    ...
    const syncVisibleVersionMetadata = () => {
        ...
        const currentHash = typeof nextData[EW_CONTENT_HASH_KEY] === "string"
    ...
    if (readResult.source === "file" && readResult.file_name) {
        const currentFileRef = typeof nextData[EW_SNAPSHOT_FILE_KEY] === "string"
            ? String(nextData[EW_SNAPSHOT_FILE_KEY]).trim()
            : "";
...
export async function rebindFloorSnapshotToMessage(settings, sourceMessageId, targetMessageId) {
    ...
    const dynSnapshots = sourceSnapshot.dyn_entries
        ...
        .map((entry) => normalizeDynSnapshot(entry));
    const controllerSnapshots = sourceSnapshot.controllers
        .map(normalizeControllerSnapshot)
        .filter((entry) => entry.content);
    await markFloorEntries(settings, targetMessageId, dynSnapshots.map((entry) => entry.name), controllerSnapshots, dynSnapshots, targetVersionInfo.swipe_id, targetVersionInfo.content_hash);
    const cleanupSwipeId = typeof sourceSnapshot.swipe_id === "number"
        ? sourceSnapshot.swipe_id
        : Number(sourceVersionInfo.swipe_id ?? 0);
    const cleanupContentHash = typeof sourceSnapshot.content_hash === "string"
...
function resolveExecutionEntryForMessage(msg) {
...
export async function migrateExecutionBetweenMessages(sourceMessageId, targetMessageId) {
    if (sourceMessageId === targetMessageId) {
...
export async function writeBindingMetaPair(sourceMessageId, targetMessageId, requestId) {
    const sourceMsg = getChatMessages(sourceMessageId)[0];
    const targetMsg = getChatMessages(targetMessageId)[0];
    if (!sourceMsg || !targetMsg) {
        return;
...
export async function migrateBeforeReplyArtifactsToAssistant(settings, sourceMessageId, targetMessageId, requestId) {
    if (sourceMessageId === targetMessageId) {
        return {
            migrated: false,
    ...
    const sourceSnapshotRead = await readSnapshotForMessageDetailed(sourceMsg, "strict");
    const targetSnapshotRead = await readSnapshotForMessageDetailed(targetMsg, "strict");
    ...
    if (sourceSnapshotRead.snapshot) {
        if (targetSnapshotRead.snapshot) {
            ...
            const snapshotMove = await rebindFloorSnapshotToMessage(settings, sourceMessageId, targetMessageId);
    ...
    if (sourceExecution) {
        if (targetExecution) {
            ...
            const executionMove = await migrateExecutionBetweenMessages(sourceMessageId, targetMessageId);
...
async function markLegacyUserAnchor(messageId, reason) {
    const msg = getChatMessages(messageId)[0];
    if (!msg) {
        return;
    }
...
function buildLocalizationSignature(messages) {
    return messages
        .map((msg) => {
        const fileRef = typeof msg?.data?.[EW_SNAPSHOT_FILE_KEY] === "string"
            ? String(msg.data[EW_SNAPSHOT_FILE_KEY])
            : "";
        const bindingRole = typeof msg?.data?.[EW_BEFORE_REPLY_BINDING_KEY]?.role === "string"
...
export async function localizeSnapshotsForCurrentChat(settings) {
    ...
    const runTask = (async () => {
        ...
        const readStoreCache = new Map();
        const readStoreCached = async (fileName) => {
        ...
        for (const msg of allMessages) {
            const snapshotFile = typeof msg?.data?.[EW_SNAPSHOT_FILE_KEY] === "string"
                ? String(msg.data[EW_SNAPSHOT_FILE_KEY])
                : "";
        ...
        const readSnapshotCached = async (messageId) => {
        ...
        for (let i = 0; i < allMessages.length; i++) {
            ...
            const migrated = await migrateBeforeReplyArtifactsToAssistant(settings, source.message_id, target.message_id, "auto-localize");
        ...
        if (result.unresolved > 0) {
            console.warn(`[Evolution World] snapshot localization unresolved=${result.unresolved} for chat ${chatId}`, result.warnings);
        }
        return result;
    })();
...
export async function markFloorEntries(settings, messageId, entryNames, controllerSnapshots, dynSnapshots, swipeId, contentHash) {
    ...
    const normalizedControllerSnapshots = (controllerSnapshots ?? [])
        .map((snapshot) => normalizeControllerSnapshot({
    ...
    const hasSnapshotPayload = Boolean(normalizedControllerSnapshots.length > 0 ||
    ...
    if (!hasSnapshotPayload) {
        if (previousSnapshotFileName) {
            if (!previousSnapshotFileOwned) {
                console.info(`[Evolution World] markFloorEntries: skipped deleting foreign snapshot file "${previousSnapshotFileName}"`);
            }
            else {
...
function hasAnySnapshotReferences(messages) {
...
export function getFloorEntryNames(messageId) {
    const messages = getChatMessages(messageId);
    if (messages.length === 0) {
        return [];
    }
...
export async function collectLatestSnapshots() {
    const lastId = getLastMessageId();
    if (lastId < 0) {
    ...
    for (let i = allMessages.length - 1; i >= 0; i--) {
        ...
        for (const snap of snapshot.dyn_entries) {
            if (snap.name && typeof snap.content === "string") {
                dynMap.set(String(snap.name), normalizeDynSnapshot(snap));
            }
        }
        return {
...
export async function purgeAndRestoreForChat(settings) {
    ...
    if (dynSnapshots.size === 0 &&
        ...
        hasSnapshotRefs) {
        console.info("[Evolution World] purgeAndRestore: no valid snapshots found for current visible versions, keeping current worldbook state");
    ...
    for (const snap of dynSnapshots.values()) {
        ...
        if (existing) {
            ...
            nextEntries.push(ensureDefaultEntry(normalizedSnap.name, normalizedSnap.content, normalizedSnap.enabled, nextEntries));
        }
    }
...
export async function migrateSnapshots(direction) {
    const lastId = getLastMessageId();
    if (lastId < 0) {
        return { migrated: 0 };
    }
...
export async function collectAllFloorSnapshots() {
    const lastId = getLastMessageId();
    if (lastId < 0) {
        return [];
    }
    ...
    for (const msg of allMessages) {
        ...
        result.push({
            ...
            execution: executionState
                ? {
                    execution_status: executionState.execution_status === "skipped"
                        ? "skipped"
                        : "executed",
                    skip_reason: typeof executionState.skip_reason === "string"
...
export async function readFloorSnapshotByMessageId(messageId, mode = "history") {
    const message = getChatMessages(messageId)[0];
    if (!message) {
        return null;
    }
...
export async function applySnapshotDiffToCurrentWorldbook(settings, previousSnapshot, nextSnapshot) {
    ...
    const previousCtrlByKey = new Map((previousSnapshot?.controllers ?? []).map((snapshot) => [
        controllerSnapshotKey(snapshot),
        normalizeControllerSnapshot(snapshot),
    ]));
    const nextCtrlByKey = new Map((nextSnapshot?.controllers ?? []).map((snapshot) => [
        controllerSnapshotKey(snapshot),
        normalizeControllerSnapshot(snapshot),
    ]));
    const dynamicUpserts = _.uniq([
        ...
        ...diff.modified,
        ...diff.toggled,
    ]).filter((name) => String(name ?? "").startsWith(settings.dynamic_entry_prefix));
    for (const entryName of dynamicUpserts) {
        ...
        if (existing) {
            const hasConflict = Boolean(previous &&
        ...
        nextEntries.push(ensureDefaultEntry(normalizedDesired.name, normalizedDesired.content, normalizedDesired.enabled, nextEntries));
        applied += 1;
    }
    const dynamicDeletes = diff.deleted.filter((name) => String(name ?? "").startsWith(settings.dynamic_entry_prefix));
    ...
    for (const [key, change] of Object.entries(diff.controllersChanged)) {
        ...
        nextEntries.push(ensureDefaultEntry(entryName, desired.content, true, nextEntries));
        applied += 1;
    }
    await replaceWorldbook(target.worldbook_name, nextEntries, {
        render: "debounced",
...
export function diffSnapshots(prev, curr) {
    ...
    const prevControllers = new Map((prev?.controllers ?? []).map((snapshot) => [
        controllerSnapshotKey(snapshot),
        snapshot,
    ]));
    const currControllers = new Map(curr.controllers.map((snapshot) => [
        controllerSnapshotKey(snapshot),
        snapshot,
    ]));
    ...
    for (const key of allCtrlKeys) {
        ...
        else if (prevVal?.content !== currVal?.content ||
...
export async function rollbackToFloor(settings, targetMessageId) {
...
export async function rollbackBeforeFloor(settings, messageId) {
...
async function restoreWorldbookFromSnapshots(settings, predicate) {
    const allFloors = await collectAllFloorSnapshots();
    const dynMerged = new Map();
    ...
    for (const floor of allFloors) {
        ...
        for (const snap of floor.snapshot.dyn_entries) {
            if (snap.name && typeof snap.content === "string") {
                dynMerged.set(String(snap.name), normalizeDynSnapshot(snap));
            }
        }
    }
    const target = await resolveTargetWorldbook(settings);
...
async function onChatChanged(settings) {
    try {
...
export function initFloorBindingEvents(getSettings) {
    disposeFloorBindingEvents();
    floorBindingListenerStops.push(onEvent(EVENT_CHAT_CHANGED(), () => {
        const currentSettings = getSettings();
        if (currentSettings.enabled && currentSettings.floor_binding_enabled) {
            scheduleFloorBindingRestore(getSettings, 500);
        }
    }));
    floorBindingListenerStops.push(onEvent(EVENT_MESSAGE_DELETED(), () => {
        const currentSettings = getSettings();
        if (currentSettings.enabled && currentSettings.floor_binding_enabled) {
...
export function disposeFloorBindingEvents() {
    if (floorBindingRestoreTimer) {
        clearTimeout(floorBindingRestoreTimer);
        floorBindingRestoreTimer = null;
    }
...
//# sourceMappingURL=floor-binding.js.map