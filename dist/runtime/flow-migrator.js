...
function makeNode(moduleId, config, x, y) {
    return {
        id: makeId(),
        moduleId,
        position: { x, y },
...
function makeEdge(sourceNode, sourcePort, targetNode, targetPort) {
    return {
 ...
 */
export function migrateFlowToGraph(flow) {
    const nodes = [];
    const edges = [];
    ...
    // ═══ Source Layer ═══
    // 1. Character fields
    const charFieldsNode = makeNode("src_char_fields", {}, COL_SRC, srcRow++ * ROW_H);
    nodes.push(charFieldsNode);
    // 2. Chat history
    const chatHistoryNode = makeNode("src_chat_history", {
        context_turns: flow.context_turns ?? 8,
    }, COL_SRC, srcRow++ * ROW_H);
    nodes.push(chatHistoryNode);
    // 3. User input
    const userInputNode = makeNode("src_user_input", {}, COL_SRC, srcRow++ * ROW_H);
    nodes.push(userInputNode);
    // 4. Flow context
    const flowCtxNode = makeNode("src_flow_context", {}, COL_SRC, srcRow++ * ROW_H);
    nodes.push(flowCtxNode);
    // 5. World book (if needed by extract or exclude rules)
    let worldbookNode = null;
    if (flow.extract_rules.length > 0 || flow.exclude_rules.length > 0) {
        worldbookNode = makeNode("src_worldbook_raw", {
    ...
    let filteredMsgsNode = null;
    if (flow.extract_rules.length > 0) {
        const extractNode = makeNode("flt_context_extract", {
        ...
        }, COL_FILTER, filterRow++ * ROW_H);
        nodes.push(extractNode);
        edges.push(makeEdge(chatHistoryNode.id, "messages", extractNode.id, "msgs_in"));
        filteredMsgsNode = extractNode;
    }
    // Context exclude rules → flt_context_exclude
    if (flow.exclude_rules.length > 0) {
        const excludeNode = makeNode("flt_context_exclude", {
    ...
    // Hide messages
    const hideNode = makeNode("flt_hide_messages", {
        hide_last_n: 0,
    ...
    edges.push(makeEdge(msgsSourceForHide.id, msgsPortForHide, hideNode.id, "msgs_in"));
    // ═══ Transform Layer ═══
    // Macro replace on system prompt
    let systemPromptNode = null;
    if (flow.system_prompt) {
        const cfgSysPrompt = makeNode("cfg_system_prompt", {
            content: flow.system_prompt,
        }, COL_TRANSFORM, transformRow++ * ROW_H);
        nodes.push(cfgSysPrompt);
        const macroNode = makeNode("tfm_macro_replace", {}, COL_TRANSFORM, transformRow++ * ROW_H);
        ...
        if (customRegexNode) {
            edges.push(makeEdge(macroNode.id, "text_out", customRegexNode.id, "text_in"));
        }
    }
    // ═══ Config Layer ═══
    // API preset config
    const apiCfgNode = makeNode("cfg_api_preset", {
    ...
    // Generation options
    const genCfgNode = makeNode("cfg_generation", {
        ...flow.generation_options,
    }, COL_COMPOSE, ROW_H);
    nodes.push(genCfgNode);
    // Behavior options
    const behaviorCfgNode = makeNode("cfg_behavior", {
        ...flow.behavior_options,
    }, COL_COMPOSE, ROW_H * 2);
    nodes.push(behaviorCfgNode);
    // Timing config
    const timingNode = makeNode("cfg_timing", {
        timing: flow.timing ?? "default",
    }, COL_COMPOSE, ROW_H * 3);
    nodes.push(timingNode);
    // ═══ Compose Layer ═══
    // Prompt order
    const promptOrderNode = makeNode("cmp_prompt_order", {
    ...
    edges.push(makeEdge(charFieldsNode.id, "main", promptOrderNode.id, "components"));
    // Wire filtered messages → depth inject
    const depthInjectNode = makeNode("cmp_depth_inject", {}, COL_COMPOSE, ROW_H * 5);
    nodes.push(depthInjectNode);
    edges.push(makeEdge(promptOrderNode.id, "msgs_out", depthInjectNode.id, "messages"));
    edges.push(makeEdge(hideNode.id, "msgs_out", depthInjectNode.id, "injections"));
    // ═══ Execute Layer ═══
    // LLM Call
    const llmCallNode = makeNode("exe_llm_call", {
        use_main_api: !flow.api_url,
    }, COL_EXECUTE, 0);
    nodes.push(llmCallNode);
    edges.push(makeEdge(depthInjectNode.id, "msgs_out", llmCallNode.id, "messages"));
    ...
    edges.push(makeEdge(behaviorCfgNode.id, "options", llmCallNode.id, "behavior"));
    // Response extract (if configured)
    let responseChain = llmCallNode;
    let responsePort = "raw_response";
    if (flow.response_extract_regex) {
        const extractNode = makeNode("exe_response_extract", {
    ...
    if (flow.response_remove_regex) {
        const removeNode = makeNode("exe_response_remove", {
    ...
    edges.push(makeEdge(responseChain.id, responsePort, jsonParseNode.id, "text"));
    // ═══ Output Layer ═══
    // Floor bind
    ...
    edges.push(makeEdge(jsonParseNode.id, "parsed", wbWriteNode.id, "operations"));
    return {
 ...
 */
export function migrateAllFlows(flows) {
...
export function autoMigrateIfNeeded(settings) {
    ...
    if (flows.length === 0)
        return [];
    console.info(`[EW Migration] Auto-migrating ${flows.length} legacy flows to workbench graphs…`);
    return migrateAllFlows(flows);
}
//# sourceMappingURL=flow-migrator.js.map