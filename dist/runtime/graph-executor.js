...
export function topologicalSort(nodes, edges) {
    ...
    const dependsOn = nodes.map(() => []);
    const downstream = nodes.map(() => []);
 ...
 */
function collectNodeInputs(node, edges, nodeOutputs) {
    const inputs = {};
    for (const edge of edges) {
 ...
 */
async function executeModule(node, inputs, context) {
    ...
    switch (moduleId) {
        ...
        case 'src_serial_results': {
            ...
            return { results: collectSerialResults(context.previousResults) };
        }
        // ═══════ Filter modules ═══════
        case 'flt_wi_keyword_match': {
            ...
            const chatTexts = typeof inputs.chat_texts === 'string'
                ...
                : Array.isArray(inputs.chat_texts)
                    ? inputs.chat_texts.map((m) => m.content ?? '').join('\n')
                    : '';
...
// ── Main Executor ──
export async function executeGraph(graph, context) {
    ...
    for (const node of graph.nodes) {
        try {
            ...
            for (const port of requiredInPorts) {
                const hasConnection = graph.edges.some(e => e.target === node.id && e.targetPort === port.id);
                if (!hasConnection) {
                    errors.push({
                        nodeId: node.id,
