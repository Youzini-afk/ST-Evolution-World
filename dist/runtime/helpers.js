...
export function resolveMessageTextForVersioning(message) {
...
export function buildMessageVersionKey(swipeId, contentHash) {
...
export function getMessageVersionInfo(message) {
...
export function clampMin(value, min) {
...
export function parseJsonObject(input) {
    ...
    try {
        ...
        const pairs = _.toPairs(parsed).map(([key, value]) => [key, String(value)]);
...
export function removeSlices(text, rules) {
...
export function extractSlices(text, rules) {
    if (!text || rules.length === 0) {
        return text;
    }
    const blocks = [];
...
export function quoteSingle(input) {
    return `'${input.replaceAll("\\", "\\\\").replaceAll("'", "\\'")}'`;
}
export function sanitizeWorldbookEntrySegment(input) {
...
function buildControllerEntryBaseName(prefix, flowName, flowId) {
    const safeName = sanitizeWorldbookEntrySegment(flowName || flowId);
    return `${prefix}${safeName}`;
}
function buildControllerEntryConflictName(prefix, flowId, flowName) {
...
function buildLegacyHashedControllerEntryName(prefix, flowId, flowName) {
    const baseName = buildControllerEntryBaseName(prefix, flowName, flowId);
    const suffix = simpleHash(flowId).slice(0, 9);
    return `${baseName}__${suffix}`;
}
export function buildControllerEntryName(prefix, flowId, flowName) {
    return buildControllerEntryBaseName(prefix, flowName, flowId);
}
export function resolveControllerEntryNameMap(prefix, flows) {
...
export function buildLegacyControllerEntryName(prefix) {
    return `${prefix}Legacy_Controller__legacy`;
}
export function resolveControllerSnapshotEntryName(prefix, snapshot) {
