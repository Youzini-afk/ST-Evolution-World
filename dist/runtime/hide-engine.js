...
// Module-level state
let hideState = { lastProcessedLength: 0 };
let scheduledHideApplyTimer = null;
/**
 ...
 */
const ewHiddenIndices = new Set();
export function isEwHiddenMessageIndex(index) {
...
function syncSystemAttribute(indices, value) {
...
function syncLimiterVisibility(startIdx) {
    if (typeof $ === "undefined") {
        return;
    }
    const $chat = $("#chat");
 ...
 */
export function runFullHideCheck(settings) {
    if (!settings.enabled)
        return;
    const chat = getChat();
 ...
 */
export function runIncrementalHideCheck(settings) {
    if (!settings.enabled)
        return;
    const chat = getChat();
    if (!chat || chat.length === 0)
...
export function applyHideSettings(settings) {
    const normalized = {
...
export function scheduleHideSettingsApply(settings, delayMs = 120) {
    const snapshot = {
        enabled: Boolean(settings.enabled),
        hide_last_n: Math.max(0, Math.trunc(Number(settings.hide_last_n ?? 0) || 0)),
        limiter_enabled: Boolean(settings.limiter_enabled),
 ...
 */
export function applyFloorLimit(settings) {
...
//# sourceMappingURL=hide-engine.js.map