...
function comparePriority(lhs, rhs) {
 ...
 */
function normalizeEntryName(name, dynPrefix, ctrlPrefix) {
    if (name.startsWith(dynPrefix))
        return name;
    if (name.startsWith(ctrlPrefix))
        return name;
    return dynPrefix + name;
}
function normalizeControllerModel(model, dynPrefix, ctrlPrefix) {
    return {
        ...model,
...
export function mergeFlowResults(results, settings) {
    const sorted = [...results].sort((lhs, rhs) => comparePriority({ priority: lhs.flow.priority, flow_order: lhs.flow_order }, { priority: rhs.flow.priority, flow_order: rhs.flow_order }));
    ...
    for (const result of sorted) {
        ...
        const referencedEntries = normalizedControllerModel
            ? new Set(_.uniq([
                ...
                ...(normalizedControllerModel.activate_entries ?? []).map((entry) => entry.entry),
                ...normalizedControllerModel.rules.flatMap((rule) => rule.include_entries),
            ]))
            : new Set();
        for (const desired of worldbookOps.desired_entries) {
            const normalizedName = normalizeEntryName(desired.name, settings.dynamic_entry_prefix, settings.controller_entry_prefix);
            const next = {
                value: {
                    name: normalizedName,
            ...
            if (result.flow.dyn_write.activation_mode === "worldbook_direct" &&
        ...
        for (const removal of worldbookOps.remove_entries) {
            const normalizedName = normalizeEntryName(removal.name, settings.dynamic_entry_prefix, settings.controller_entry_prefix);
            const next = {
                value: null,
                priority,
                flow_order: flowOrder,
    ...
    const controllerEntryNameMap = resolveControllerEntryNameMap(settings.controller_entry_prefix, [...controllerModels.values()].map((slot) => ({
    ...
    for (const entry of desiredEntries) {
        ...
        if (err) {
            ...
            try {
                globalThis.toastr?.warning(`EJS 语法错误: ${entry.name}`, "Evolution World");
            }
            catch {
                /* noop */
            }
