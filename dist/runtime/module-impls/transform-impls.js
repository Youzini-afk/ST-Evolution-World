 ...
 */
export async function transformEjsRender(template, context) {
    if (!template)
        return "";
    try {
 ...
 */
export function transformMacroReplace(text) {
    if (!text)
        return "";
    try {
 ...
 */
export async function transformControllerExpand(entries) {
 ...
 *   7-9 = various depths
 */
export function transformWiBucket(entries) {
    const before = [];
    const after = [];
    const atDepth = [];
    for (const entry of entries) {
 ...
 */
export function transformEntryNameInject(messages, snapshots) {
...
//# sourceMappingURL=transform-impls.js.map