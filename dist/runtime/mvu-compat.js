...
const MVU_FORMAT_EMPHASIS_ENTRY_REGEX = /(?:变量输出格式强调|格式强调[：:]?-?变量更新规则|格式强调[：:]?-?剧情演绎|The following must be inserted to the end of (?:each )?reply,? and cannot be omitted)[\s\S]*?format:\s*\|-?/i;
...
export function isLikelyMvuWorldInfoContent(content) {
    ...
    return (MVU_STATUS_CURRENT_VARIABLE_DETECT_REGEX.test(normalized) ||
...
export function stripBlockedPromptContents(content, blockedContents) {
    ...
    const normalizedBlocked = _.uniq(blockedContents
        .map(item => item.trim())
        .filter(Boolean)
...
//# sourceMappingURL=mvu-compat.js.map