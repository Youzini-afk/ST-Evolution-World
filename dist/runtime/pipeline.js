...
function inferFailureSuggestion(stage, kind, reason) {
    const httpStatus = extractHttpStatus(reason);
    switch (kind) {
        case "auth_error":
...
function classifyDispatchFlowFailure(reason) {
...
function shouldRunFlowOnRound(flow, round) {
...
function resolveAutoTriggerAnchorMessageId(input) {
    const assistantMessageId = Number(input.trigger?.assistant_message_id);
    if (input.timing_filter === "after_reply" &&
...
function resolveAutoTriggerOrdinal(input) {
...
function buildWorkflowFailureDiagnostic(params) {
    ...
    return {
        ...
        whole_workflow_failed: attempts.length > 0 &&
            failedAttempts.length === 0 &&
            stage !== "dispatch",
    };
}
function toPreview(value, maxLen = 3000) {
    try {
 ...
 */
function sanitizeRequestDebug(debug) {
    const copy = klona(debug);
    if (copy.transport_request?.custom_api &&
    ...
    if (typeof copy.transport_request?.custom_include_headers === "string") {
        copy.transport_request.custom_include_headers =
            copy.transport_request.custom_include_headers.replace(/(Authorization\s*:\s*Bearer\s+)\S+/gi, "$1[REDACTED]");
    }
    return copy;
}
function buildAttemptRequestPreview(attempt) {
    const debug = attempt.request_debug ?? { flow_request: attempt.request };
    return toPreview(sanitizeRequestDebug(debug), 20000);
}
function saveIoSummary(requestId, chatId, mode, attempts) {
    setLastIo({
        at: Date.now(),
        request_id: requestId,
        chat_id: chatId,
...
function persistIoSummarySafe(requestId, chatId, mode, attempts) {
    try {
...
function persistRunSummarySafe(summary) {
    try {
        setLastRun(summary);
    }
    catch (error) {
...
function isWorkflowCancelled(input) {
    return Boolean(input.abortSignal?.aborted || input.isCancelled?.());
}
function throwIfWorkflowCancelled(input) {
    if (isWorkflowCancelled(input)) {
        throw new Error("workflow cancelled by user");
    }
}
async function waitWithCancellation(ms, input) {
    if (ms <= 0) {
        return;
    }
...
function extractStructuredWriteback(diagnostics) {
...
async function runGraphWorkflow(input, settings, enabledGraphs, requestId, startedAt) {
...
export async function runWorkflow(input) {
    ...
    try {
        throwIfWorkflowCancelled(input);
        const workbenchGraphs = settings.workbench_graphs ?? [];
        ...
        let enabledFlows = selectedFlowIds.size > 0
            ? allEnabledFlows.filter((flow) => selectedFlowIds.has(flow.id))
            : allEnabledFlows;
        if (input.timing_filter) {
            enabledFlows = enabledFlows.filter((flow) => {
                const effective = flow.timing === "default" ? settings.workflow_timing : flow.timing;
        ...
        const dispatchOutput = await withTimeout(dispatchFlows({
            settings,
            flows: enabledFlows,
        ...
        const commitResult = await commitMergedPlan(settings, mergedPlan, controllerTemplates, requestContext.request_id ?? requestId, requestContext.message_id);
        throwIfWorkflowCancelled(input);
        ...
        const summary = RunSummarySchema.parse({
            ...
            request_id: requestId,
            chat_id: currentChatId,
            flow_count: (input.flow_ids?.length ?? 0) > 0
