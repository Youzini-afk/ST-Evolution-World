...
import { collectIgnoredWorldInfoContents, resolveWorldInfo, } from "./worldinfo-engine";
...
function resolveCharacterCardFieldsGetter() {
    ...
    if (typeof ctx?.getCharacterCardFields === "function") {
        return {
            getter: ctx.getCharacterCardFields,
            source: "ctx.getCharacterCardFields",
        };
    }
    ...
    if (typeof hostRuntime.SillyTavern?.getCharacterCardFields ===
        "function") {
        return {
            getter: hostRuntime.SillyTavern.getCharacterCardFields,
            source: "hostRuntime.SillyTavern.getCharacterCardFields",
        };
    }
    if (typeof getCharacterCardFields === "function") {
        return {
            getter: getCharacterCardFields,
            source: "global getCharacterCardFields",
        };
    }
    return {};
}
function hasCharacterCardFieldValue(fields) {
    if (!fields) {
        return false;
    }
    return Boolean(fields.description?.trim() ||
...
function getRuntimeCharacterCardFields() {
    return resolveRuntimeCharacterCardFields().fields;
}
function getRuntimeCharacterId() {
    const hostRuntime = getHostRuntime();
    const ctx = getRuntimeContext();
    const candidates = [
        ctx?.characterId,
...
function getRuntimeCharData() {
    ...
    if (Number.isFinite(characterId) &&
        characterId >= 0 &&
        runtimeCharacters[characterId]) {
        return {
            value: runtimeCharacters[characterId],
            source: `runtimeCharacters[${characterId}]`,
        };
    }
    if (Number.isFinite(Number(ctx?.characterId)) &&
        Number(ctx?.characterId) >= 0 &&
        Array.isArray(ctx?.characters)) {
        return {
            value: ctx.characters[Number(ctx.characterId)] ??
                null,
...
function getRuntimeLastMessageId() {
    const hostRuntime = getHostRuntime();
...
function getRuntimeChatMessages(range, opts) {
    const hostRuntime = getHostRuntime();
...
function resolveWorkflowChatRole(msg) {
    if (msg?.role === "system" ||
        msg?.role === "user" ||
        msg?.role === "assistant") {
        return msg.role;
...
function resolveWorkflowChatText(msg) {
...
function collectProcessedWorkflowChatMessages(lastId, settings) {
    if (lastId < 0) {
...
function collectRawWorkflowChatMessages(lastId, settings) {
    if (lastId < 0) {
        return { messages: [], total_count: 0, filtered_hidden_count: 0 };
    }
...
function detectCompressedWorkflowChatHistory(processedMessages, rawCount) {
    ...
    const suspiciouslyFewMessages = rawCount >= 6 &&
        processedMessages.length <= Math.max(2, Math.floor(rawCount / 4));
    if (containsKnownMarkers && allSystem) {
        return {
            detected: true,
            reason: "宿主聊天历史命中了压缩相邻消息包装标记，且当前结果只剩 system 块。",
        };
    }
    if (containsKnownMarkers && suspiciouslyFewMessages && dialogueCount === 0) {
        return {
            detected: true,
            reason: "宿主聊天历史被压缩包装后只剩少量 system 块，缺少正常 user/assistant 轮次。",
        };
    }
    if (additionalSettingsCount >= 2 &&
        suspiciouslyFewMessages &&
        dialogueCount === 0) {
        return {
            detected: true,
            reason: "宿主聊天历史主要由 additional_settings 包装壳组成，已判定为压缩污染。",
        };
    }
    return { detected: false };
}
function describeAttempt(label, value, detail) {
    if (typeof value === "string") {
...
function resolveTextCandidate(candidates, note) {
...
function appendDiagnosticNote(diagnostic, note) {
    if (!diagnostic) {
        return diagnostic;
    }
    return {
...
function sanitizeWorkflowExtensionPrompt(content, blockedContents = []) {
    const sanitized = stripBlockedPromptContents(stripMvuPromptArtifacts(content), blockedContents);
    if (isLikelyMvuWorldInfoContent(content) ||
        isLikelyMvuWorldInfoContent(sanitized)) {
        return "";
    }
    return sanitized;
}
...
function stripWorkflowImageBlocks(content) {
    if (!content.trim()) {
...
function sanitizeWorkflowChatMessage(content, settings) {
...
function formatDiagnosticBlock(diagnostic, rawLength, renderedLength) {
    if (!diagnostic) {
        return [
            `原始长度: ${rawLength}`,
...
function getRuntimePersonaDescription() {
    const hostRuntime = getHostRuntime();
    const ctx = getRuntimeContext();
    return resolveTextCandidate([
        {
            label: "ctx.powerUserSettings.persona_description",
            value: ctx?.powerUserSettings?.persona_description,
        },
        { label: "ctx.persona_description", value: ctx?.persona_description },
        {
            label: "hostRuntime.power_user.persona_description",
            value: hostRuntime.power_user?.persona_description,
        },
        {
            label: "globalThis.power_user.persona_description",
            value: globalThis.power_user?.persona_description,
        },
        {
            label: "power_user.persona_description",
            value: power_user?.persona_description,
        },
        {
            label: "getCharacterCardFields().persona",
            value: getRuntimeCharacterCardFields()?.persona,
        },
...
function getRuntimeCharacterFields() {
    ...
    const main = resolveTextCandidate([
        ...
        {
            label: "charData.data.system_prompt",
            value: charData?.data?.system_prompt,
        },
    ], "主系统提示词来源");
    const jailbreak = resolveTextCandidate([
        {
            label: "getCharacterCardFields().jailbreak",
            value: helperFields?.jailbreak,
        },
        {
            label: "charData.data.post_history_instructions",
            value: charData?.data?.post_history_instructions,
        },
    ], "后历史指令来源");
    const charDescription = resolveTextCandidate([
        {
            label: "getCharacterCardFields().description",
            value: helperFields?.description,
        },
        { label: "charData.description", value: charData?.description },
        {
            label: "charData.data.description",
            value: charData?.data?.description,
        },
        { label: "ctx.name2_description", value: ctx?.name2_description },
    ], "角色描述多路径回退");
    const charPersonality = resolveTextCandidate([
        {
            label: "getCharacterCardFields().personality",
            value: helperFields?.personality,
        },
        { label: "charData.personality", value: charData?.personality },
        {
            label: "charData.data.personality",
            value: charData?.data?.personality,
        },
        { label: "ctx.name2_personality", value: ctx?.name2_personality },
    ], "角色性格多路径回退");
    const scenario = resolveTextCandidate([
        {
            label: "getCharacterCardFields().scenario",
            value: helperFields?.scenario,
        },
    ...
    const dialogueExamples = resolveTextCandidate([
        {
            label: "getCharacterCardFields().mesExamples",
            value: helperFields?.mesExamples,
        },
        { label: "charData.mes_example", value: charData?.mes_example },
        {
            label: "charData.data.mes_example",
            value: charData?.data?.mes_example,
        },
    ], "示例对话字段回退");
    ...
    return {
        ...
        diagnostics: {
            ...
            postHistoryInstructions: appendDiagnosticNote(jailbreak.diagnostic, [
                helperResult.note,
                `helper=${helperSource ?? "none"}; ${charDataState}`,
            ]
                .filter(Boolean)
                .join("; ")),
            charDescription: appendDiagnosticNote(charDescription.diagnostic, [
                helperResult.note,
                `helper=${helperSource ?? "none"}; ${charDataState}`,
            ]
                .filter(Boolean)
                .join("; ")),
            charPersonality: appendDiagnosticNote(charPersonality.diagnostic, [
                helperResult.note,
                `helper=${helperSource ?? "none"}; ${charDataState}`,
            ]
                .filter(Boolean)
                .join("; ")),
            scenario: appendDiagnosticNote(scenario.diagnostic, [
                helperResult.note,
                `helper=${helperSource ?? "none"}; ${charDataState}`,
            ]
                .filter(Boolean)
                .join("; ")),
            personaDescription: appendDiagnosticNote(personaDescription.diagnostic, [
                helperResult.note,
                `helper=${helperSource ?? "none"}; ${charDataState}`,
            ]
                .filter(Boolean)
                .join("; ")),
            dialogueExamples: appendDiagnosticNote(dialogueExamples.diagnostic, [
                helperResult.note,
                `helper=${helperSource ?? "none"}; ${charDataState}`,
            ]
                .filter(Boolean)
...
async function populateWorldInfoComponents(components, settings) {
    components.blockedWorldInfoContents = await collectIgnoredWorldInfoContents();
    try {
        const chatTexts = components.chatMessages
            .map((msg) => msg.content)
            .filter(Boolean);
        ...
        components.diagnostics.worldInfoBefore = appendDiagnosticNote(components.diagnostics.worldInfoBefore, `world info 读取异常: ${String(e)}`);
        components.diagnostics.worldInfoAfter = appendDiagnosticNote(components.diagnostics.worldInfoAfter, `world info 读取异常: ${String(e)}`);
 ...
 */
export async function collectPromptComponents(flow, settings, contextCursor) {
    const components = {
        main: "",
        jailbreak: "",
        charDescription: "",
    ...
    try {
        ...
        if (lastId >= 0) {
            ...
            const shouldFallbackToRaw = contextMode === "host_processed" &&
            ...
            const selectedSourceMode = shouldPreferRaw
                ...
                : shouldFallbackToRaw
                    ? "raw_chat_fallback"
                    : "host_processed";
            const selectedMessages = selectedSourceMode === "host_processed" || rawResult.total_count === 0
            ...
            components.diagnostics.chatHistory = {
                selectedSource: selectedSourceMode === "host_processed"
                    ? "getChatMessages()"
                    : selectedSourceMode === "raw_chat_fallback"
                        ? "ctx.chat(raw) ← fallback"
                        : "ctx.chat(raw)",
                attempts: chatHistoryAttempts,
                note: selectedSourceMode === "raw_chat_fallback"
                ...
                fallback_reason: selectedSourceMode === "raw_chat_fallback"
                    ? (compressionCheck.reason ?? "宿主聊天结果不可用")
                    : undefined,
                raw_count: rawResult.total_count,
                processed_count: processedResult.total_count,
                filtered_hidden_count: selectedSourceMode === "host_processed"
            ...
            components.diagnostics.chatHistory = {
                ...
                source_mode: settings?.workflow_chat_context_mode === "raw_chat_preferred"
                    ? "raw_chat"
                    : "host_processed",
                raw_count: 0,
                processed_count: 0,
        ...
        components.diagnostics.chatHistory = appendDiagnosticNote(components.diagnostics.chatHistory, `聊天记录读取异常: ${String(e)}`);
    }
    // ── 3. World Info (before/after) ───────────────────────────────────────
    await populateWorldInfoComponents(components, settings ?? {});
    ...
    try {
        ...
        const extPrompts = ctx2?.extensionPrompts ??
            getHostRuntime().extension_prompts ??
        ...
        if (extPrompts && typeof extPrompts === "object") {
            ...
            if (promptAreaEntries.length) {
                ...
                components.diagnostics.worldInfoBefore = {
                    ...
                    selectedSource: components.diagnostics.worldInfoBefore?.selectedSource ??
                        "extensionPrompts(IN_PROMPT)",
                    attempts: [
                        ...(components.diagnostics.worldInfoBefore?.attempts ?? []),
                        {
            ...
            if (components.beforePromptInjections.length) {
                components.diagnostics.worldInfoBefore = appendDiagnosticNote(components.diagnostics.worldInfoBefore, `检测到 ${components.beforePromptInjections.length} 条 BEFORE_PROMPT 扩展提示词，已放到请求最前面`);
            }
        }
    }
    catch (e) {
...
function createMarkerPreviewMessage(title, content) {
    return {
        role: "system",
        content,
        debugOnly: true,
...
async function buildMarkerPreviewMessage(entry, components, options = {}) {
    const markerTitle = entry.name?.trim() || entry.identifier;
    const diagnostic = components.diagnostics[entry.identifier];
    if (entry.identifier === "chatHistory") {
        ...
        return createMarkerPreviewMessage(`📌 ${markerTitle}`, [summary, formatDiagnosticBlock(diagnostic, count, count)]
            .filter(Boolean)
            .join("\n\n"));
    }
    const rawContent = resolveMarkerContent(entry.identifier, components);
    const renderedContent = rawContent.trim()
        ? await renderEjsContent(rawContent, options.templateContext)
        : "";
    const rawLength = rawContent.trim().length;
    const renderedLength = renderedContent.trim().length;
    const summary = renderedLength > 0
        ? `已读取该段内容（渲染后 ${renderedLength} chars）。触发工作流时会发送。`
        : rawLength > 0
            ? "已读取原始内容，但 EJS 渲染后为空。触发工作流时这里不会发送正文。"
            : "当前为空，因此这里不会发送任何内容。";
    return createMarkerPreviewMessage(`📌 ${markerTitle}`, [summary, formatDiagnosticBlock(diagnostic, rawLength, renderedLength)]
        .filter(Boolean)
        .join("\n\n"));
}
/**
 ...
 */
export async function assembleOrderedPrompts(promptOrder, components, options = {}) {
    const result = [];
    ...
    for (const entry of promptOrder) {
        ...
        if (entry.injection_position === "in_chat" &&
            ...
            if (entry.type === "prompt" && entry.content.trim()) {
                ...
                const content = await renderEjsContent(resolveMarkerContent(entry.identifier, components), options.templateContext);
                if (content.trim()) {
                    deferredInjections.push({
                        content,
                        role: entry.role,
        ...
        if (entry.type === "marker") {
            ...
            if (entry.identifier === "worldInfoBefore" ||
                entry.identifier === "worldInfoAfter") {
 ...
 */
function resolveMarkerContent(identifier, components) {
    switch (identifier) {
        case "main":
 ...
 */
export async function previewPrompt(flow, settings) {
