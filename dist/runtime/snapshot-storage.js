 ...
 *
 * Synced from script version: upgraded to ew-message-store/v3
 *  - workflow_execution & replay_capsules external artifact maps
 *  - deterministic archived version key generation
 *  - structured pruning of archived revisions
 *  - backward-compatible read of ew-snapshot/v2 stores
 */
 ...
 * Now also normalizes dyn_entries through normalizeDynSnapshotData.
 */
export function upgradeSnapshotData(raw) {
    if (!raw || typeof raw !== "object")
        return null;
    const dynEntries = Array.isArray(raw.dyn_entries)
        ? raw.dyn_entries
            .map((entry) => normalizeDynSnapshotData(entry))
    ...
    if (raw.controllers &&
        ...
        return {
            controllers: Object.entries(raw.controllers).map(([key, value]) => ({
...
export function buildArchivedSnapshotVersionKey(baseKey, store, revisionStamp = Date.now()) {
...
export function buildArchivedArtifactVersionKey(baseKey, map, revisionStamp = Date.now()) {
    let candidate = `${baseKey}@rev:${revisionStamp}`;
    let counter = 0;
    while (map[candidate]) {
        counter += 1;
...
export function pruneArchivedVersionedEntries(map, baseKey, keepRecent = FILE_ARTIFACT_REVISION_LIMIT) {
    const archivedEntries = Object.keys(map)
        .map((key) => ({ key, parsed: parseArchivedVersionKey(key) }))
        .filter((entry) => entry.parsed !== null && entry.parsed.baseKey === baseKey)
        .sort((left, right) => {
...
export function pruneAllVersionedEntries(map, keepRecent = FILE_ARTIFACT_REVISION_LIMIT) {
...
// ---------------------------------------------------------------------------
// Store owner & fingerprint
// ---------------------------------------------------------------------------
function buildChatFingerprint(chatId) {
    return simpleHash(String(chatId ?? ""))
        .replace(/^h/, "")
        .slice(0, 12);
}
function buildSnapshotStoreOwner(charName, chatId) {
    return {
...
function normalizeSnapshotStoreOwner(raw) {
...
// ---------------------------------------------------------------------------
// External artifact version map normalization
// ---------------------------------------------------------------------------
function normalizeExternalArtifactVersionMap(raw) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
        return {};
    }
    const result = {};
...
// Empty store factory
// ---------------------------------------------------------------------------
function createEmptyStore(owner) {
    return {
...
// ---------------------------------------------------------------------------
// Store normalization (backward-compatible: reads v2 and v3)
// ---------------------------------------------------------------------------
function normalizeSnapshotVersionStore(raw) {
    if (!raw || typeof raw !== "object") {
        return null;
    }
    // Current version: ew-message-store/v3
    if (raw.version === FILE_ARTIFACT_VERSION &&
        raw.versions &&
    ...
    // Legacy version: ew-snapshot/v2 — upgrade to v3
    if (raw.version === "ew-snapshot/v2" &&
        raw.versions &&
        typeof raw.versions === "object" &&
        !Array.isArray(raw.versions)) {
        const versions = {};
    ...
    // Bare snapshot object — wrap in v3 store
...
// ---------------------------------------------------------------------------
// Store cloning & external artifact accessors
// ---------------------------------------------------------------------------
...
export function getExternalArtifactMap(store, kind) {
    if (!store) {
        return {};
    }
    return {
        ...(kind === "workflow_execution"
            ? store.workflow_execution
            : store.replay_capsules),
    };
}
export function setExternalArtifactMap(store, kind, map) {
...
export function hasSnapshotStorePayload(store) {
    if (!store) {
        return false;
    }
    return Boolean(Object.keys(store.versions).length > 0 ||
...
// Write
// ---------------------------------------------------------------------------
async function persistSnapshotStore(fileName, store) {
...
export async function writeSnapshotStore(fileName, store) {
...
export async function writeSnapshot(charName, chatId, messageId, data) {
    const fileName = buildFileName(charName, chatId, messageId);
    const currentStore = (await readSnapshotStore(fileName)) ??
        createEmptyStore(buildSnapshotStoreOwner(charName, chatId));
...
// Read
// ---------------------------------------------------------------------------
export async function readSnapshotStore(fileName) {
    try {
...
export async function readSnapshot(fileName, versionKey) {
 ...
 */
export async function findSnapshotFiles(charName, chatId, messageIds) {
 ...
 */
export async function cleanupSnapshotFiles(charName, chatId, allMessageIds, keepFileNames) {
...
// ---------------------------------------------------------------------------
// Re-exports for use by other modules (floor-binding, events, etc.)
// ---------------------------------------------------------------------------
export { buildChatFingerprint, buildFileName, buildFilePrefix, buildLegacyFileName, buildLegacyFilePrefix, buildSnapshotStoreOwner, createEmptyStore as createEmptySnapshotStore, };
// Test-only export (same pattern as script version)
export { buildArchivedSnapshotVersionKey as buildArchivedSnapshotVersionKeyForTest };
//# sourceMappingURL=snapshot-storage.js.map