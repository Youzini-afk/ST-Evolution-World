...
import { applyDynWriteConfigToEntry, buildDynSnapshotFromEntry, createDynEntryFromWriteConfig, ensureDefaultEntry, resolveTargetWorldbook, } from "./worldbook-runtime";
function isManagedEntryName(settings, name) {
    if (name.startsWith(settings.controller_entry_prefix)) {
        return true;
    }
    return name.startsWith(settings.dynamic_entry_prefix);
}
function compareContributionApplyOrder(lhs, rhs) {
    if (lhs.priority !== rhs.priority) {
        return lhs.priority - rhs.priority;
    }
    return lhs.flow_order - rhs.flow_order;
}
function parseMarkdownItemSet(raw) {
...
function normalizeMarkdownItem(item) {
    return String(item ?? "")
        .replace(/\s+/g, " ")
        .trim();
}
function dedupeMarkdownItems(items) {
    const seen = new Set();
    const result = [];
...
function renderMarkdownItemSet(set) {
...
function applyMarkdownMerge(currentContent, incomingContent, mode) {
...
function pickWinningContribution(contributions) {
    return [...contributions].sort(compareContributionApplyOrder)[contributions.length - 1];
}
function groupDesiredEntries(entries) {
...
function materializeDynEntryContent(entryName, currentContent, contributions) {
    ...
    if (ordered.every((entry) => entry.dyn_write.mode === "add")) {
        ...
        for (const contribution of ordered) {
            ...
            if (!merged.ok) {
                console.warn(`[EW Commit] skip incremental Dyn "${entryName}" from flow "${contribution.source_flow_id}": ${merged.reason}`);
...
function applyResolvedManagedEntries(nextEntries, resolvedEntries) {
...
function collectManagedDynSnapshots(nextEntries, settings) {
    return nextEntries
        .filter((entry) => entry.name.startsWith(settings.dynamic_entry_prefix))
        .map((entry) => buildDynSnapshotFromEntry(entry));
}
function collectManagedControllerSnapshots(nextEntries, settings) {
    return nextEntries
...
export async function commitMergedPlan(settings, mergedPlan, controllerTemplates, _requestId, messageId) {
    ...
    if (settings.floor_binding_enabled && messageId >= 0) {
        ...
        await markFloorEntries(settings, messageId, dynSnapshots.map((entry) => entry.name), controllerSnapshots, dynSnapshots, versionInfo.swipe_id, versionInfo.content_hash);
    }
    return {
