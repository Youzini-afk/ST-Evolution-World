...
import { createWorldbook, getCharWorldbookNames, getWorldbook, rebindCharWorldbooks, } from "./compat/worldbook";
import { DynWorldbookProfileSchema, } from "./types";
function toEntrySnapshot(entries) {
...
function normalizePositionRole(raw) {
    return raw === "user" || raw === "assistant" ? raw : "system";
}
function normalizeSecondaryLogic(raw) {
    return raw === "and_all" || raw === "not_any" || raw === "not_all"
        ? raw
        : "and_any";
}
function normalizeScanDepth(raw) {
...
function normalizeNullableInt(raw) {
...
export function getDefaultDynWorldbookProfile() {
    return DynWorldbookProfileSchema.parse({});
}
export function extractDynWorldbookProfile(raw) {
    return DynWorldbookProfileSchema.parse({
        ...
        strategy: raw?.strategy
            ? {
                type: typeof raw.strategy.type === "string" && raw.strategy.type.trim()
...
export function normalizeDynSnapshotData(raw) {
...
export function buildDynSnapshotFromEntry(entry) {
...
export function applyDynSnapshotToEntry(entry, snapshot) {
...
export function applyDynWriteConfigToEntry(entry, name, content, dynWrite) {
...
export function createDynEntryFromWriteConfig(name, content, entries, dynWrite) {
...
export function createDynEntryFromSnapshot(snapshot, entries) {
    const normalized = normalizeDynSnapshotData(snapshot);
    if (!normalized) {
 ...
 */
export async function resolveTargetWorldbook(_settings) {
 ...
 */
export async function getFullWorldbookContext(preloadedTarget) {
...
export function ensureDefaultEntry(name, content, enabled, entries, _constant = false) {
    return {
        uid: nextUid(entries),
        name,
        enabled,
...
//# sourceMappingURL=worldbook-runtime.js.map