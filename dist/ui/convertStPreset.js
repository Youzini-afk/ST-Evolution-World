 ...
 */
export function isSillyTavernPreset(obj) {
 ...
 */
export function convertStPresetToFlow(preset, name) {
    // ── 1. 生成参数 ──
    const genOpts = {};
    if (typeof preset.temperature === 'number')
        genOpts.temperature = preset.temperature;
    if (typeof preset.top_p === 'number')
        genOpts.top_p = preset.top_p;
    if (typeof preset.frequency_penalty === 'number')
        genOpts.frequency_penalty = preset.frequency_penalty;
    if (typeof preset.presence_penalty === 'number')
        genOpts.presence_penalty = preset.presence_penalty;
    if (typeof preset.openai_max_context === 'number')
        genOpts.max_context_tokens = preset.openai_max_context;
    if (typeof preset.openai_max_tokens === 'number')
        genOpts.max_reply_tokens = preset.openai_max_tokens;
    if (typeof preset.stream_openai === 'boolean')
        genOpts.stream = preset.stream_openai;
    if (typeof preset.max_context_unlocked === 'boolean')
        genOpts.unlock_context_length = preset.max_context_unlocked;
    // ── 2. prompt_order 构建 ──
    //
    // ST 预设结构：
    ...
    const stPrompts = preset.prompts;
    let promptOrder;
    if (Array.isArray(stPrompts) && stPrompts.length > 0) {
        ...
        const stOrder = preset.prompt_order;
        let orderList = null;
        if (Array.isArray(stOrder) && stOrder.length > 0) {
            ...
            if (Array.isArray(inner)) {
                orderList = inner
                    .filter((e) => typeof e?.identifier === 'string')
                    .map((e) => ({
        ...
        const seen = new Set();
        const result = [];
        const convertPrompt = (identifier, enabledOverride) => {
            if (seen.has(identifier))
                return null; // 去重
            seen.add(identifier);
            const p = promptMap.get(identifier);
            if (!p)
                return null; // 定义不存在则跳过
            ...
            return {
                ...
                role: (['system', 'user', 'assistant'].includes(p.role)
                    ? p.role : 'system'),
    ...
    if (extensions && typeof extensions === 'object' && !Array.isArray(extensions)) {
        const sp = extensions.SPreset;
        if (sp && typeof sp === 'object' && !Array.isArray(sp)) {
            const rb = sp.RegexBinding;
            if (rb && typeof rb === 'object' && !Array.isArray(rb) && Array.isArray(rb.regexes)) {
                for (const r of rb.regexes) {
                    customRegex.push({
...
//# sourceMappingURL=convertStPreset.js.map