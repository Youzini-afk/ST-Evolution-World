...
function ensureWorkflowStyle(doc) {
...
function ensureWorkflowHost(doc) {
...
function applyNoticeState(item, input, progress) {
    ...
    const actionButton = item.querySelector(".ew-floating-notice__action");
    const actionWrap = item.querySelector(".ew-floating-notice__actions");
...
export function showManagedEwNotice(input) {
...
export function showEwNotice(input) {
    showManagedEwNotice(input);
}
function applyWorkflowNoticeState(item, input, progress) {
    const level = input.level ?? "info";
    ...
    const actionButton = item.querySelector(".ew-workflow-notice__action");
    const actionWrap = item.querySelector(".ew-workflow-notice__actions");
    if (actionButton && actionWrap) {
        if (input.action) {
            actionWrap.style.display = "";
            actionButton.style.display = "";
    ...
    if (islandContent) {
        ...
        const textSpan = islandContent.querySelector(".ew-workflow-notice__island-text");
        if (textSpan) {
    ...
    const orb = item.querySelector(".ew-workflow-notice__island-orb");
    if (orb) {
    ...
    const extraBadge = item.querySelector(".ew-workflow-notice__island-extra");
    if (extraBadge) {
...
export function showManagedWorkflowNotice(input) {
    const initialDuration = Math.max(1400, input.duration_ms ?? 3200);
    const doc = resolveNoticeDocument();
    ...
    const scheduleCollapse = (nextInput) => {
