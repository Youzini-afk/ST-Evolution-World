...
import { checkMinimumVersion as _checkMinVersion, substituteParams, } from "../runtime/compat/tools";
export function assignInplace(destination, new_array) {
    destination.length = 0;
    destination.push(...new_array);
    return destination;
}
// 修正 _.merge 对数组的合并逻辑, [1, 2, 3] 和 [4, 5] 合并后变成 [4, 5] 而不是 [4, 5, 3]
export function correctlyMerge(lhs, rhs) {
...
export function regexFromString(input, replace_macros) {
    if (!input) {
        return null;
    }
    const makeRegex = (pattern, flags) => {
        if (replace_macros) {
            pattern = substituteParams(pattern);
        }
...
export async function checkMinimumVersion(expected, title) {
    _checkMinVersion(expected, title);
}
export function prettifyErrorWithInput(error) {
...
export function literalYamlify(value) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    const toError = (error) => error instanceof Error
