...
export function readCharFlowDraft(charName: string): EwFlowConfig[] | null {
  ...
  try {
    ...
    console.warn(
      "[Evolution World] Failed to read char flow draft cache:",
      error,
    );
    return null;
  }
...
export function writeCharFlowDraft(
  charName: string,
  flows: EwFlowConfig[],
): void {
  const storageKey = getCharFlowDraftStorageKey(charName);
  ...
  try {
    ...
    console.warn(
      "[Evolution World] Failed to write char flow draft cache:",
      error,
    );
  }
}
...
export function clearCharFlowDraft(charName: string): void {
  ...
  try {
    ...
    console.warn(
      "[Evolution World] Failed to clear char flow draft cache:",
      error,
    );
  }
}
 ...
 * 如果条目不存在或解析失败，返回空数组。
 */
export async function readCharFlows(
  settings: EwSettings,
): Promise<EwFlowConfig[]> {
  try {
    ...
    for (const raw of (parsed as CharFlowsPayload).flows) {
      try {
        ...
        if (!flow.api_preset_id || !presetIds.has(flow.api_preset_id)) {
          const recoveredPresetId = globalPresetIdByName.get(
            normalizeFlowName(flow.name),
          );
 ...
 * - 合并后按 priority 降序排列
 */
export async function getEffectiveFlows(
  settings: EwSettings,
): Promise<EwFlowConfig[]> {
  ...
  try {
    ...
    const draftFlows = currentCharName
      ...
      : null;
    charFlows = (draftFlows ?? (await readCharFlows(settings))).filter(
      (f) => f.enabled,
    );
  } catch {
    charFlows = [];
  ...
  const charFlowNames = new Set(
    charFlows.map((f) => normalizeFlowName(f.name)),
  );

  const filteredGlobal = globalFlows.filter(
    (flow) =>
      !charFlowIds.has(flow.id) &&
