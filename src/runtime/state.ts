...
export function clearSendContextIfMatches(
...
export function pruneExpiredBeforeReplyBindingPending(
  currentAt = now(),
): BeforeReplyBindingPending | null {
...
export function markBeforeReplyBindingMigrated(
...
export function wasAfterReplyHandled(
  message_id: number,
  content: string,
): boolean {
  ...
  return (
    state.after_reply.last_handled_hash === contentHash &&
...
export function isQuietLike(
  type: string,
  params: { quiet_prompt?: string } | undefined,
): boolean {
  if (type === "quiet") {
    return true;
  }
...
export function shouldHandleGenerationAfter(
  ...
  const hasFreshSend = Boolean(
    lastSend && now() - lastSend.at <= settings.gate_ttl_ms,
  );
  const hasFreshIntent = Boolean(
    lastIntent && now() - lastIntent.at <= settings.gate_ttl_ms,
  );
...
export function shouldHandleAfterReply(
  ...
  const hasFreshGeneration = Boolean(
    state.last_generation && now() - state.last_generation.at <= windowMs,
  );
